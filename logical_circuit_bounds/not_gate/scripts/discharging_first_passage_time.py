...
in the discharging (1 in -> 0 out) direction.
"""
import numpy as np
...
absorbing_inds_rev = [np.where(-gate_vals[_]/Vd_vec[_]/Cg >= (1-alpha))[0] for _ in range(len(Vd_vec))]

# Get the initial state for all cross-voltages
for state_ind in range(len(Vd_vec)):

    # Set an initial state for Vin = 1
    init_state = np.zeros((2, 2, Ngate[state_ind]))
    ind = np.where(gate_vals[state_ind] == 0)[0]
    init_state[0, 0, ind] = 1.
    init_state = init_state.reshape(-1)
    state.append(init_state.copy())

    # Get a time evolution operator for the opposite input
    paramsi = {'beta': 1.,
               'gamma': 1.,
               'Cg': Cg,
               'Vd': Vd_vec[state_ind],
    ...
    # Now start doing an evolution back in the other direction for Vin = 0
    # and continue this evolution until the probability of not being
    # within alpha of the correct measurment is less than alpha_tol
    for step in range(ntsteps_init):
        ...
        # Do a measurement to see what the initial state looks like
        vl = np.ones(state[state_ind].shape)
        initial_density[state_ind] = measure_gate_occ(state[state_ind],
                                                      vl,
                                                      gate_vals[state_ind])

        # Break the loop if we have crossed the accuracy threshold
...
absorbing_inds = [np.where(-gate_vals[_]/Vd_vec[_]/Cg <= (alpha))[0] for _ in range(len(Vd_vec))]
...
for step in range(ntsteps):
    ...
    if plot:
        ...
        plt.tight_layout()
        plt.pause(0.01)
        plt.savefig(f'./data/first_passage_time_distribution_reverse_Cg{Cg}_alpha{alpha}.pdf')
    np.savez(f'./data/first_passage_time_distribution_reverse_Cg{Cg}_alpha{alpha}.npz',
             t=tvec[:step+2],
             S=S,
             state = state,
             current = current,
