...
# Plot evolution
f, ax = plt.subplots(1, 1, figsize=(4*(3/4), 2.5*(3/4)))

# Plot 1 - ACF
lw=2
ax.plot(t_vec, -gate_densities[:, 0]/(Cg*Vd), '-', color=cmap(0.1), linewidth=lw)
ax.plot(t_vec, -gate_densities[:, 1]/(Cg*Vd), '-', color=cmap(0.4), linewidth=lw)
ax.plot(t_vec, -gate_densities[:, 2]/(Cg*Vd), '-', color=cmap(0.7), linewidth=lw)
ax.set_ylim(0., 1.)
ax.set_xlim(0, t_vec[-1])
ax.set_yticks([0, 1])
plt.savefig(f'./data/example_trajectory_Cg{Cg}_Vd{Vd}.png', dpi=500)
